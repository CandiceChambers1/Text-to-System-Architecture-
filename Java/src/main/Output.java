...
public class Output {
    ...
    public String generateContent(){
        String output = "\t<XMI.content>\n"+
                "\t\t<UML:Model name=\"EA Model\" xmi.id=\"" + xmi_id + "\">\n"+
			    "\t\t\t<UML:Namespace.ownedElement>\n"+
				"\t\t\t\t<UML:Class name=\"EARootClass\" xmi.id=\"" + xmi_root_id + "\"/>"+
                "\t\t\t\t<UML:Package name=\"One Level Block Hierarchy\" xmi.id=\"" + xmi_package_id + "\">\n" +
                "\t\t\t\t\t<UML:Namespace.ownedElement>";
    ...
    public String generateAssociation(){

        String output = "<UML:Association xmi.id=\"" + xmi_id + "\">\n"+
            "\t\t<UML:ModelElement.taggedValue>\n"+
            ...
            "\t\t\t<UML:AssociationEnd type=\"" + xmi_id + "\">\n" +
            "\t\t\t</UML:AssociationEnd>\n" +
            "\t\t\t<UML:AssociationEnd type=\""+ xmi_id + "\">\n"+
            "\t\t</UML:AssociationEnd>";
...
//        if direction == TRUE
    ...
    public String generateDirection(){
        String output = "\t<UML:TaggedValue tag=\"direction\" xmi.id=\"" + xmi_id + "\" value=\"" + direction + "#NOTES#Values: in,out,inout,none;\" modelElement=\"" + xmi_id+ "\"/>\n";
        return output;
    }

    public String generateEncapsulation(){
        String output = "\t<UML:TaggedValue tag=\"isEncapsulated\" xmi.id=\"" + xmi_id + "\" value=\"#NOTES#Values: true,false&#xA;\" modelElement=\"" + xmi_id + "\"/>\n";
        return output;
    }
    ...
    public generateDiagramBDD(){
        ...
        String output = "\t<UML:Diagram name = \"One Level Block Hierarchy\" xmi.id=\"" + xmi_id + "\" diagramType = \"ClassDiagram\" owner=\"" + xmi_id + "\" toolName = \"Enterprise Architect 2.5\">\n" +
                "\t\t<UML:ModelElement.taggedValue>\n" +
                "\t\t\t<UML:TaggedValue tag=\"package\" value=\"" + xmi_package + "\">\n"+
                "\t\t\t<UML:TaggedValue tag=\"styleex\" value=\"MDGDgm=SysML1.4::BlockDefinition;SF=1;\"/>\n"+
                "\t\t</UML:ModelElement.taggedValue>\n"+
                "\t\t<UML:Diagram.element>\n";
                for(amount of figures to be drawn)
                    if block():
                        output += "\t\t\t<UML:DiagramElement geometry=\"Left=500;Top=150;Right=690;Bottom=190;\" subject=\"" + xmi_id + "\"/>\n";
                    if port():
                        output += "\t\t\t<UML:DiagramElement geometry=\"Left=300;Top=400;Right=400;Bottom=300;\" subject=\"" + xmi_id + "\"/>\n";
    ...
    public generateDiagramIBD(){
        String output = "\t<UML:Diagram name=\"" + noun +"\" xmi.id=\"" + xmi_id+ "\" diagramType=\"CompositeStructureDiagram\" owner=\"" + xmi_package + "\" toolName=\"Enterprise Architect 2.5\">\n"+
                "\t\tUML:ModelElement.taggedValue>\n"+
                "\t\t\t<UML:TaggedValue tag=\"package\" value=\"" + xmi_package + "\"/>\n"+
				"\t\t\t<UML:TaggedValue tag=\"type\" value=\"CompositeStructure\"/>\n"+
				"\t\t\t<UML:TaggedValue tag=\"ea_localid\" value=\"2\"/>\n" +
				"\t\t\t<UML:TaggedValue tag=\"parent\" value=\"" + xmi_id + "\"/>\n" +
				"\t\t\t<UML:TaggedValue tag=\"styleex\" value=\"MDGDgm=SysML1.4::InternalBlock;SF=1;\"/>"+
                "\t\t</UML:ModelElement.taggedValue>\n"+
                "\t\t<UML:Diagram.element>\n";
                for(amount of figures to be drawn):
                    if port()
                        output += "\t\t\t<UML:DiagramElement geometry=\"Left=543;Top=202;Right=558;Bottom=217;\" subject=\"" + xmi_id + "\"/>\n";
                    if property():
                        output += "\t\t\t<UML:DiagramElement geometry=\"Left=699;Top=129;Right=714;Bottom=144;\" subject=\"" + xmi_id + "\"/>\n";
                    if connection():
                        output += "\t\t\t<UML:DiagramElement geometry=\"SX=0;SY=0;EX=0;EY=0;EDGE=2;$LLB=;LLT=;LMT=;LMB=;LRT=;LRB=;IRHS=;ILHS=;Path=;\" subject=\"" + xmi_id + "\"/>\n";
